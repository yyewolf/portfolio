...
class Game {
  ...
  private repaired = 0;
  /**
   * Counted where the mistakes are made rather than where they show up. A
   * restart of a healthy server settles straight back to running, so measuring
   * consequences at read time missed the exact behaviour levels three and four
   * exist to catch, and blamed the player for failures the script caused.
   *
   * `needless` is a change made when the last reading matched and nothing had
   * arrived since. `blind` is a change made with events still unread. They
   * cannot both be true of one move.
   */
  private stats = { changes: 0, reads: 0, needless: 0, blind: 0 };
  /** `needless + blind` for the current level, which picks the debrief. */
  private levelSlips = 0;
  ...
  private go(where: string): void {
    switch (where) {
      case "start":
        this.stats = { changes: 0, reads: 0, needless: 0, blind: 0 };
        this.cleared.clear();
        this.startLevel(0);
        break;
      case "next":
  ...
  private move(kind: ActionKind, machine?: string): void {
    ...
    // Read before applying: a move can push its own entries onto the feed, and
    // the question here is what the player knew when they clicked, not after.
    const matched = this.seenGap() === 0;
    const behind = this.staleness() > 0;
  ...
  private renderDebrief(): void {
    ...
    if (!this.refs) return;
    // Levels whose answer is "leave it alone" can be cleared by acting anyway,
    // because a needless restart settles back to running and the count still
    // matches. Praising that play for restraint it did not show is the one
    // thing a lesson built on self-discovery cannot afford to do.
    const lines = this.levelSlips > 0 && s.debriefActed ? s.debriefActed : s.debrief;
    ...
    this.refs.cards.innerHTML = `
      <div class="op-card op-in">
        ...
        ${lines.map((p, i) => `<p${i === 0 ? ` class="op-lede"` : ""}>${esc(p)}</p>`).join("")}
        <div class="op-foot">
  ...
  private renderDone(): void {
    ...
    // A floor rather than a target. Level four can hand you a reading that
    // honestly asks for an action, and taking it is not a mistake.
    const par = scenarios.reduce((n, s) => n + s.par, 0);
    const score = (label: string, value: number, note: string): string =>
      `<div><dt>${label}</dt><dd>${value}<small>${note}</small></dd></div>`;
    ...
    this.screen(`
      ${this.loopDiagram()}
      <dl class="op-scores">
        ${score("Changes you made", this.stats.changes, `fewest possible: ${par}`)}
        ${score("Times you read the state", this.stats.reads, "the move every level was testing")}
        ${score("Changes with nothing to fix", this.stats.needless, "your last reading already matched")}
        ${score("Changes made before looking", this.stats.blind, "events were sitting unread")}
      </dl>
      <p class="op-scores-note">Reading is the only number here meant to be high. It costs nothing,
      and it is the one move that can end a level. The bottom two are what a change costs when
      there is no reading behind it.</p>
